...
def pre_order_travers(tree: RBTree, root: Node, status: Status, curr_depth: int) -> None:
    ...
    if root.has_left_son():
        ...
        if root.left_node.key > root.key:
            raise BinarySearchTreePropertiesMismatch('The key value in the left subtree '
                                                     'is greater than the root key')
    ...
    if root.has_right_son():
        ...
        if root.right_node.key < root.key:
            raise BinarySearchTreePropertiesMismatch('The key value in the right subtree '
                                                     'is less than in the root key')
